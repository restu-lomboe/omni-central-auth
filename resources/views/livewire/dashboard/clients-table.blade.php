            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($clients as $client)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-gray-500 max-w-xs truncate">{{ $client->redirect }}</td>
                        <td class="px-6 py-4">
                            @if ($client->revoked)
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-700">Revoked</span>
                            @else
                        ...
                        <td class="px-6 py-4 text-right space-x-3">
                            ...
                            <button wire:click="revokeClient({{ $client->id }})"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-gray-400 text-sm">
                            Belum ada OAuth client. <a href="{{ route('omni.dashboard.clients.create') }}" class="text-omni-500">Buat sekarang →</a>
                        </td>
                    </tr>
                @endforelse
            </tbody>
